...
public final class KRActivityIndicatorView: UIView {
    ...
    @IBInspectable fileprivate(set) var headColor: UIColor = UIColor.black {
        willSet {
            if largeStyle {
    ...
    /// If you change color, change activityIndicatorViewStyle property.
    @IBInspectable fileprivate(set) var tailColor: UIColor = UIColor.lightGray {
        willSet {
            if largeStyle {
    ...
    public var isAnimating: Bool {
        if animationLayer.animation(forKey: "rotate") != nil { return true }
        else { return false }
    }
