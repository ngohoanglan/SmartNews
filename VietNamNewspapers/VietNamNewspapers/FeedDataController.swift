...
class FeedDataController {
    ...
    func getFeedDataByTitleAndDescriptionKeyWord(keyWord: NSString, description:NSString) -> Array<FeedData> {
        var fetchedResults:Array<FeedData> = Array<FeedData>()
        ...
        //Add a predicate to filter by eventId
        let findByIdPredicate =
            NSPredicate(format: "\(FeedDataAttributes.title) CONTAINS[cd] %@ OR \(FeedDataAttributes.feedDescription) CONTAINS[cd] %@", keyWord,description)
        fetchRequest.predicate = findByIdPredicate
    ...
    func deleteFeedDataAfterDay(_ afterDay:Int)
    {
        let minionManagedObjectContextWorker: NSManagedObjectContext =
            NSManagedObjectContext.init(concurrencyType: NSManagedObjectContextConcurrencyType.privateQueueConcurrencyType)
        minionManagedObjectContextWorker.parent = self.mainContextInstance
