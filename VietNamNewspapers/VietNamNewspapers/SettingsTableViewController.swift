...
class Settings
{
    ...
    func getCountryCodeSelectedKey() -> String {
       ...
       // var countryCode = NSLocalizedString("country_code", comment: "")
        return ""
    }
    ...
    func getTextSize()  -> Float{
        ...
        if(UIDevice.current.userInterfaceIdiom == .phone){
            ...
            {
                return sliderTextSizeKeyDefaultValue-1
            }
            ...
            {
                return sliderTextSizeKeyDefaultValue+2
            }
            else if(DeviceType.IS_IPHONE_8X)
            {
        ...
        else
        {
            ...
            {
                return sliderTextSizeKeyDefaultValue+4
            }
            else if(DeviceType.IS_IPAD_PRO_10_5)
            {
                 return sliderTextSizeKeyDefaultValue+4.5
            }
            else if(DeviceType.IS_IPAD_PRO)
            {
                return sliderTextSizeKeyDefaultValue+5
            }
            else
            {
                return sliderTextSizeKeyDefaultValue+5
    ...
    //TableRowHeight
    func getPercentAddCellHeightBaseOnDevice()  -> CGFloat{
        ...
        let percent: CGFloat = (SCREEN_MAX_LENGTH-568.0)/568.0*100/2
        return percent
        /*
        
        let defaultPercent:CGFloat=0.0
        if(UIDevice.current.userInterfaceIdiom == .phone){
            ...
            {
                return defaulSize+10
            }
            else if(DeviceType.IS_IPHONE_6P_7P)
            {
                return defaulSize+15
            }
            else if(DeviceType.IS_IPHONE_8X)
            {
                return defaulSize+20
            }
            else
            {
                return defaulSize
        ...
        else
        {
            if(DeviceType.IS_IPAD)
            {
                return defaulSize+30
            }
            else if(DeviceType.IS_IPAD_PRO_10_5)
            {
                return defaulSize+40.5
            }
            else if(DeviceType.IS_IPAD_PRO)
            {
                return defaulSize+45
            }
            else
            {
                return defaulSize+45
...
public  struct DeviceType
{
    ...
    static let IS_IPHONE_8X         = UIDevice.current.userInterfaceIdiom == .phone && ScreenSize.SCREEN_MAX_LENGTH == 812.0
...
class SettingsTableViewController: UITableViewController ,UIGestureRecognizerDelegate, SKProductsRequestDelegate, SKPaymentTransactionObserver {
   let settings=Settings()
    ...
    @objc func taplbDeleteAllNewspapersAction(_ gr:UITapGestureRecognizer)
    {
       

        
        let alert = UIAlertController(title: NSLocalizedString("delete_all_newspaper", comment: ""), message: NSLocalizedString("confirm_delete_all_newspaper", comment: ""), preferredStyle: UIAlertController.Style.alert)
        ...
        alert.addAction(UIAlertAction(title: "Ok", style: .default, handler: { action in
            ...
            case .default:
                ...
                alertMess.addAction(UIAlertAction(title: "OK", style: UIAlertAction.Style.default, handler: nil))
    ...
    @objc func taplbRemoveAdsAction(_ gr:UITapGestureRecognizer)
    {
        let payment = SKPayment(product: productsArray[0])
        SKPaymentQueue.default().add(payment)
    }
    // In-App Purchase Methods
    @objc func taplbRestorePurchaseAction(_ gr:UITapGestureRecognizer)
    {
        SKPaymentQueue.default().add(self)
    ...
    @objc func taplbDeleteDataAction(_ gr:UITapGestureRecognizer)
    {
        let alert = UIAlertController(title: NSLocalizedString("clear_cache", comment: ""), message: NSLocalizedString("clear_cache_confirm", comment: ""), preferredStyle: UIAlertController.Style.alert)
        alert.addAction(UIAlertAction(title: NSLocalizedString("cancel", comment: ""), style: UIAlertAction.Style.default, handler: nil))
        self.present(alert, animated: true, completion: nil)
        alert.addAction(UIAlertAction(title: "Ok", style: .default, handler: { action in
            switch action.style{
